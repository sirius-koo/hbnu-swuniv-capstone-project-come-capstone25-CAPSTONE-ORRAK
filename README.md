...
| :---: | :---: |
| 안정 상태 | 불안정 상태 |
- 왼쪽 사진과 같이 무게 중심(COM)이 기저면(BOS) 내부에 위치할 경우 녹색으로,  
  오른쪽처럼 COM이 기저면 밖으로 벗어나면 빨간색으로 시각화된다.  
- 상태를 시각적으로 표현하는 UI를 제공할 뿐, 자세 교정 피드백은 사용자가 직접 수행한다.
  예를 들어, 사용자는 불안정 상태 영상을 보고 “왼발을 벽에 두어 삼지점을 만들면 더 안정적이겠다”는  
  판단을 내리며 자세를 교정할 수 있다.
